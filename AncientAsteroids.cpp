#pragma once
// Include Files
#include "AncientAsteroids.h"

// Game Engine Functions
BOOL GameInitialize(HINSTANCE hInstance)
{
	// Create the game engine
	g_pGame = new GameEngine(hInstance, LPTSTR(TEXT("Ancient Asteroids")),
		LPTSTR(TEXT("Ancient Asteroids")), IDI_ASTEROIDICON, IDI_ASTEROIDICON, 600, 450);
	if (g_pGame == NULL)
		return FALSE;
...
void GameStart(HWND hWindow)
{
	...
	g_pGame->PlayMIDISong(LPTSTR(TEXT("Music.mid")));
...
void GameActivate(HWND hWindow)
{
	...
	g_pGame->PlayMIDISong(LPTSTR(TEXT("")), FALSE);
}
...
{
	// See if a missile and an asteroid have collided
	if ((pSpriteHitter->GetBitmap() == g_pMissileBitmap &&
		pSpriteHittee->GetBitmap() == g_pAsteroidBitmap) ||
		(pSpriteHitter->GetBitmap() == g_pAsteroidBitmap &&
	...
	// See if an asteroid has collided with a city
	if (pSpriteHitter->GetBitmap() == g_pAsteroidBitmap &&
...
{
	// See if an asteroid sprite is dying
	if (pSpriteDying->GetBitmap() == g_pAsteroidBitmap)
	{
		// Play the explosion sound
		PlaySound((LPCSTR)IDW_EXPLODE, g_hInstance, SND_ASYNC |
			SND_RESOURCE | SND_NOSTOP);

		// Create an explosion sprite at the asteroid's position
		RECT rcBounds = { 0, 0, 600, 450 };
...
void AddAsteroid()
{
	// Create a new Asteroid sprite and set its position
	RECT    rcBounds = { 0, 0, 600, 390 };
	int     iXPos = rand() % 600;
	Sprite* pSprite = new Sprite(g_pAsteroidBitmap, rcBounds, BA_DIE);
